...
public class Spielst extends Game {
    ...
    public void setPresence(OnlineStatus status, String state, String details, String application_id,
        ...
        game.put("timestamps",new JSONObject().put("start",System.currentTimeMillis()));
        ...
        game.put("since",System.currentTimeMillis());
        data.put("game",game);
        ((JDAImpl) Bot.getInstance().getBot()).getClient().send((new JSONObject()).put("d", data).put("op", 3).toString());
